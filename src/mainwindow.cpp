...
//const char COMMUNITY_FORUM_URL[] = "https://vuecashtalk.org";
const char COMMUNITY_FORUM_URL[] = "https://vuecash.org";
const char REPORT_ISSUE_URL[] = "https://vuecash.org/contact";
    ...
    , walletModel_(walletModel)
{
    ...
    setWindowIcon(QIcon(":images/vuecash_lin"));
    clearTitle();
...
//    qDebug("raw tx = %s", qPrintable(tx.binary_transaction));
    ...
    for (const RpcApi::Transfer& tf: tx.transaction.transfers)
    {
        ...
        msg.append(tr("%1 VUE to %2\n").arg(amountStr).arg(addressStr));
    }
    msg.append(tr("Fee: %1 VUE\n").arg(formatAmount(tx.transaction.fee)));
    msg.append(tr("Total send: %1 VUE").arg(formatAmount(-ourAmount)));
