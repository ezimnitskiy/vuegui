...
{
    setApplicationName("vuecash"); // do not change becasuse it also changes data directory under Mac and Win
    setApplicationDisplayName(tr("Vuecash Wallet"));
...
bool WalletApplication::init()
{
    ...
    QString path = dataDir.absoluteFilePath("vuecash-gui.lock");
    m_lockFile.reset(new QLockFile(path));

    if (!m_lockFile->tryLock())
    {
        WalletLogger::warning(tr("[Application] Vuecash wallet GUI already running"));
        QMessageBox::warning(nullptr, QObject::tr("Fail"), "Vuecash wallet GUI already running");
        return false;
    }
...
void WalletApplication::initSystemTrayIcon()
{
    ...
    m_systemTrayIcon->setIcon(QIcon(":images/vuecash_lin"));
...
{
    splashMsg(tr("Looks like this is your first run of the new Vuecash Wallet GUI. It is in detached state now.\nYou can open a wallet file, create a new one, or connect to remote walletd daemon."));
}
...
//    const QString msg = metaEnum.valueToKey(static_cast<int>(exitCode));
    ...
    switch(exitCode)
    {
    case static_cast<int>(BuiltinWalletd::ReturnCode::VUECASHD_DATABASE_ERROR):
        msg = tr("Database write error. Disk is full or database is corrupted.");
        break;
    case static_cast<int>(BuiltinWalletd::ReturnCode::VUECASHD_ALREADY_RUNNING):
        msg = tr("Cannot run vuecashd. Another instance of vuecashd is running.");
        break;
    case static_cast<int>(BuiltinWalletd::ReturnCode::WALLETD_BIND_PORT_IN_USE):
        ...
        break;
    case static_cast<int>(BuiltinWalletd::ReturnCode::VUECASHD_BIND_PORT_IN_USE):
        msg = tr("Cannot run vuecashd. Vuecashd bind port in use.");
        break;
...
//        showPasswordEdit = true;
    ...
    case static_cast<int>(BuiltinWalletd::ReturnCode::WALLET_FILE_EXISTS):
        msg = tr("The specified wallet file already exists. Vuecash wallet could not overwrite an existed file for safety reason. If you want to overwrite the file please remove it manually and try again.");
        break;
